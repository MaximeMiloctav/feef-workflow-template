...
<template>
  <UPageCard class="w-full py-0 h-full" spotlight>
    <template #default>
      <div class="flex flex-row gap-2 items-start w-full">
        ...
        <div class="flex flex-col flex-1 min-w-0 w-full">
          ...
          <div class="flex flex-row gap-1 mt-1 flex-wrap min-h-[20px]">
            <UBadge v-if="auditInitial && auditInitial > 0" class="bg-blue-100 text-blue-700 border-none text-xs px-1.5 py-0.5">{{
              auditInitial }} Initial</UBadge>
            <UBadge v-if="auditRenouvellement && auditRenouvellement > 0" class="bg-purple-100 text-purple-700 border-none text-xs px-1.5 py-0.5">{{
              auditRenouvellement }} Renouvellement</UBadge>
            <UBadge v-if="auditSuivi && auditSuivi > 0" class="bg-green-100 text-green-700 border-none text-xs px-1.5 py-0.5">{{
